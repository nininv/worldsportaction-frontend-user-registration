...
export function* getInvoiceSaga(action) {
    try {
        const result = yield call(AxiosApi.getInvoice, action.registrationid, action.userRegId, action.invoiceId);
        if (result.status === 1) {
            yield put({
...
export function* getInvoiceStatusSaga(action) {
    try {
        ...
        const result = yield call(AxiosApi.getInvoiceStatus, action.registrationid, action.userRegId, action.invoiceId);
        if (result.status === 1) {
            yield put({
