...
class AppTeamRegistrationForm extends Component {
    ...
    onChangeTeamMemberValue = (value, key, index, subIndex, parentIndex) => {
        ...
        if (key == 'mobileNumber') {
            ...
            if (value.length == 10) {
                this.props.updateRegistrationTeamMemberAction(value, key, index, subIndex)
                let hasError = this.state.hasErrorEmergencyTeamMember;
                let obj = hasError.find(element => element.teamMemberIndex == index);
                if (obj != undefined) {
                    hasError.splice(hasError.indexOf(obj), 1);
                }
                this.setState({
                    hasErrorEmergencyTeamMember: hasError
                })
                ...
                if (!regexNumberExpression(value)) {
                    setTimeout(() => {
                        this.props.form.setFieldsValue({
                            [`teamMemberEmergencyContactNumber${index}`]: null,
                        });
                    }, 300);
                }
            }
        ...
        try {
            ...
            if (key === "isSameAddress") {
                ...
                if (value.length === 10) {
                    ...
                    teamRegistrationObj.parentOrGuardian[parentIndex]["mobileNumber"] = regexNumberExpression(value);
                    this.props.updateTeamRegistrationStateVarAction(teamRegistrationObj, "teamRegistrationObj");
                } else if (value.length < 10) {
                    teamRegistrationObj.parentOrGuardian[parentIndex]["mobileNumber"] = regexNumberExpression(value);
                    this.props.updateTeamRegistrationStateVarAction(teamRegistrationObj, "teamRegistrationObj");
    ...
    teamMemberParentOrGuardianView = (parent, parentIndex, teamMember, teamMemberIndex, getFieldDecorator) => {
        let hasErrorTeamMemberParent = this.state.hasErrorTeamMemberParent;
        let hasErrorTeamMemberParentviaIndex = hasErrorTeamMemberParent.find(element => element.parentIndex == parentIndex && element.teamMemberIndex == teamMemberIndex);
        let hasError = false;
    ...
    teamMemberView = (teamMember, teamMemberIndex, getFieldDecorator) => {
        ...
        let hasErrorTeamMemberviaIndex = this.state.hasErrorTeamMember.find(element => element.teamMemberIndex == teamMemberIndex);
        let hasError = false;
    ...
    teamMemberEmergencyContactView = (teamMemberIndex, getFieldDecorator) => {
        let hasErrorTeamMemberviaIndex = this.state.hasErrorEmergencyTeamMember.find(element => element.teamMemberIndex == teamMemberIndex);
        let hasError = false;
        if (hasErrorTeamMemberviaIndex != undefined) {
            hasError = hasErrorTeamMemberviaIndex.error;
        }
